...
* @returns {ScaleResult}
*/
export function axisHelper(userOptions: ScaleOption): ScaleResult {
  const option = {
  ...
  function countDegree(step: number): void {
    axisMax = parseInt(`${dataMax as number / step + 1}`) * step; // parseInt令小数去尾 -1.8 -> -1
    axisMin = parseInt(`${dataMin as number / step - 1}`) * step;
